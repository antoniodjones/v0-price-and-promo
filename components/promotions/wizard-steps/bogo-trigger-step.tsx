...
// Mock data for different selection types
const mockItems = [
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="triggerQuantity">Trigger Quantity</Label>
          <Input
            id="triggerQuantity"
            type="number"
            min="1"
            value={formData.triggerQuantity}
            onChange={(e) => updateFormData({ triggerQuantity: Number.parseInt(e.target.value) || 1 })}
          />
          <p className="text-xs text-muted-foreground">How many items must be purchased to trigger the offer</p>
        </div>
      </div>
          ...
          {getSelectionData().map((item: any) => (
            <div
              ...
              className={`p-3 border-b last:border-b-0 cursor-pointer hover:bg-gray-50 transition-colors ${
                formData.triggerTarget === item.name ? "bg-gti-light-green/10 border-l-4 border-l-gti-bright-green" : ""
              }`}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {formData.triggerTarget === item.name && (
      ...
      {formData.type === "category" && formData.triggerTarget && (
        <div className="space-y-3">
          <Label>Subcategories (Optional)</Label>
          <p className="text-sm text-muted-foreground">
            Narrow down to specific subcategories within {formData.triggerTarget}
          </p>
          <div className="flex flex-wrap gap-2">
            {mockCategories
              .find((cat) => cat.name === formData.triggerTarget)
      ...
      {formData.triggerTarget && (
        <Card className="bg-gti-light-green/10 border-gti-bright-green">
          ...
          <CardContent>
            <p className="text-sm">
              When customers purchase <span className="font-semibold">{formData.triggerQuantity}</span> of{" "}
              <span className="font-semibold text-gti-dark-green">{formData.triggerTarget}</span>
