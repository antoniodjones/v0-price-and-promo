...
-- Version 3: Made created_by nullable to avoid foreign key issues
...
CREATE TABLE volume_pricing_rules (
  ...
  created_by UUID, -- Made nullable, no foreign key constraint
...
CREATE TABLE pricing_applications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  customer_id UUID, -- Made nullable, no foreign key constraint
  product_id UUID, -- Made nullable, no foreign key constraint
